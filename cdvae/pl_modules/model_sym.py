...
class CDVAE_SYM(BaseModule):
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        print('used_sigmas_per_atom: ', used_sigmas_per_atom.shape)

        type_noise_level = torch.randint(0, self.type_sigmas.size(0),
                                         (batch.num_atoms.size(0),),
                                         device=self.device)
        print('type_noise_level: ', type_noise_level.shape)
        used_type_sigmas_per_atom = (
            self.type_sigmas[type_noise_level].repeat_interleave(
                batch.num_atoms, dim=0))
        print('used_type_sigmas_per_atom: ', used_type_sigmas_per_atom.shape)
        ...
        print('pred_composition_probs: ', pred_composition_probs.shape)
        print('atom_type_probs: ', atom_type_probs.shape)
        print('rand_atom_types: ', rand_atom_types.shape)
        ...
        print('cart_noises_per_atom: ', cart_noises_per_atom.shape)
        print('cart_coords: ', cart_coords.shape)
        print('noisy_frac_coords: ', noisy_frac_coords.shape)
        print('pred_cart_coord_diff: ', pred_cart_coord_diff.shape)
        print('pred_atom_types: ', pred_atom_types.shape)
        # compute loss.
