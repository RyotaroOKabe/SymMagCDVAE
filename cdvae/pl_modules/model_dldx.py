...
class CDVAE_SYM(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.fc_num_atoms_lyr = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                      self.hparams.fc_num_layers, self.hparams.max_atoms+1) #!
        self.fc_num_atoms_otr = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                      self.hparams.fc_num_layers, self.hparams.max_atoms+1) #!
        self.fc_lattice = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                    self.hparams.fc_num_layers, 6)
        self.fc_composition_lyr = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                        self.hparams.fc_num_layers, MAX_ATOMIC_NUM) #!
        self.fc_composition_otr = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                        self.hparams.fc_num_layers, MAX_ATOMIC_NUM) #!
    ...
    def decode_stats(self, z, gt_num_atoms_lyr=None, gt_num_atoms_otr=None, 
                     gt_lengths=None, gt_angles=None, teacher_forcing=False):    #TODO
        """
        decode key stats from latent embeddings.
        batch is input during training for teach-forcing.
        """
        if gt_num_atoms_lyr is not None and gt_num_atoms_lyr is not None:
            num_atoms_lyr, num_atoms_otr = self.predict_num_atoms(z)   #TODO
            lengths_and_angles, lengths, angles = (
                self.predict_lattice(z, gt_num_atoms_lyr, gt_num_atoms_otr))    #TODO
            composition_per_atom_lyr, composition_per_atom_otr = self.predict_composition(z, gt_num_atoms_lyr, gt_num_atoms_otr)
            if self.hparams.teacher_forcing_lattice and teacher_forcing:
                lengths = gt_lengths
                angles = gt_angles
        else:
            num_atoms_lyr, num_atoms_lyr = self.predict_num_atoms(z).argmax(dim=-1)
            lengths_and_angles, lengths, angles = (
                self.predict_lattice(z, num_atoms_lyr, num_atoms_otr))  #TODO
            composition_per_atom_lyr, composition_per_atom_otr = self.predict_composition(z, num_atoms_lyr, num_atoms_otr)

        return num_atoms_lyr, num_atoms_otr, lengths_and_angles, lengths, angles, composition_per_atom_lyr, composition_per_atom_otr

    @torch.no_grad()
    def langevin_dynamics(self, z, ld_kwargs, gt_num_atoms_lyr=None, gt_num_atoms_otr=None, gt_atom_types_lyr=None, gt_atom_types_otr=None):   #TODO
        """
        decode crystral structure from latent embeddings.
        ld_kwargs: args for doing annealed langevin dynamics sampling:
            n_step_each:  number of steps for each sigma level.
        ...
        if ld_kwargs.save_traj:
            all_frac_coords_lyr = []
            all_frac_coords_otr = []
            all_pred_cart_coord_diff_lyr = []
            all_pred_cart_coord_diff_otr = []
            all_noise_cart_lyr = []
            all_noise_cart_otr = []
            all_atom_types_lyr = []
            all_atom_types_otr = []
        ...
        # obtain key stats.
        num_atoms_lyr, num_atom_otr, _, lengths, angles, N_lyr, \
            composition_per_atom_lyr, composition_per_atom_otr = self.decode_stats(
            z, gt_num_atoms_lyr, gt_num_atoms_otr)    #TODO
        if gt_num_atoms_lyr is not None:
            num_atoms_lyr = gt_num_atoms_lyr
        if gt_num_atoms_otr is not None:
            num_atoms_otr = gt_num_atoms_otr

        # obtain atom types.
        composition_per_atom_lyr = F.softmax(composition_per_atom_lyr, dim=-1)
        composition_per_atom_otr = F.softmax(composition_per_atom_otr, dim=-1)
        if gt_atom_types_lyr is None:
            cur_atom_types_lyr = self.sample_composition(composition_per_atom_lyr, num_atoms_lyr)    #TODO ? 
        else:
            cur_atom_types_lyr = gt_atom_types_lyr
        if gt_atom_types_lyr is None:
            cur_atom_types_otr = self.sample_composition(composition_per_atom_otr, num_atoms_otr)    #TODO ? 
        else:
            cur_atom_types_otr = gt_atom_types_otr

        # init coords.
        cur_frac_coords_lyr = torch.rand((num_atoms_lyr.sum(), 3), device=z.device) #TODO put the atoms on the layer
        cur_frac_coords_otr = torch.rand((num_atoms_otr.sum(), 3), device=z.device)

        # annealed langevin dynamics.
        for sigma in tqdm(self.sigmas, total=self.sigmas.size(0), disable=ld_kwargs.disable_bar):   #? can I always use the same sigma?
            if sigma < ld_kwargs.min_sigma:
                break
            step_size = ld_kwargs.step_lr * (sigma / self.sigmas[-1]) ** 2

            for step in range(ld_kwargs.n_step_each):
                noise_cart_lyr = torch.randn_like(
                    cur_frac_coords_lyr) * torch.sqrt(step_size * 2)
                noise_cart_otr = torch.randn_like(
                    cur_frac_coords_otr) * torch.sqrt(step_size * 2)
                pred_cart_coord_diff_lyr, pred_cart_coord_diff_otr, pred_atom_types_lyr, pred_atom_types_otr = self.decoder(
                    z, cur_frac_coords_lyr, cur_frac_coords_otr, cur_atom_types_lyr, cur_atom_types_otr, num_atoms_lyr, num_atoms_otr, lengths, angles)
                cur_cart_coords_lyr= frac_to_cart_coords(cur_frac_coords_lyr, lengths, angles, num_atoms_lyr) #TODO
                pred_cart_coord_diff_lyr = pred_cart_coord_diff_lyr / sigma
                pred_cart_coord_diff_otr = pred_cart_coord_diff_otr / sigma
                cur_cart_coords_lyr = cur_cart_coords_lyr + step_size * pred_cart_coord_diff_lyr + noise_cart_lyr
                cur_cart_coords_otr = cur_cart_coords_otr + step_size * pred_cart_coord_diff_lyr + noise_cart_lyr
                cur_frac_coords_otr = cart_to_frac_coords(cur_cart_coords_lyr, lengths, angles, num_atom_otr) #TODO

                if gt_atom_types_lyr is None:
                    cur_atom_types_lyr = torch.argmax(pred_atom_types_lyr, dim=1) + 1
                if gt_atom_types_otr is None:
                    cur_atom_types_otr = torch.argmax(pred_atom_types_otr, dim=1) + 1

                if ld_kwargs.save_traj:
                    all_frac_coords_lyr.append(cur_frac_coords_lyr)
                    all_frac_coords_otr.append(cur_frac_coords_otr)
                    all_pred_cart_coord_diff_lyr.append(step_size * pred_cart_coord_diff_lyr)
                    all_pred_cart_coord_diff_otr.append(step_size * pred_cart_coord_diff_otr)
                    all_noise_cart_lyr.append(noise_cart_lyr)
                    all_noise_cart_otr.append(noise_cart_otr)
                    all_atom_types_lyr.append(cur_atom_types_lyr)
                    all_atom_types_otr.append(cur_atom_types_otr)

        output_dict = {'num_atoms_lyr': num_atoms_lyr, 'num_atoms_otr': num_atoms_otr, 'lengths': lengths, 'angles': angles,
                       'frac_coords': cur_frac_coords_lyr, 'frac_coords': cur_frac_coords_otr, 
                       'atom_types': cur_atom_types_otr, 'atom_types': cur_atom_types_otr,
                       'is_traj': False}

        if ld_kwargs.save_traj:
            output_dict.update(dict(
                all_frac_coords_lyr=torch.stack(all_frac_coords_lyr, dim=0),
                all_frac_coords_otr=torch.stack(all_frac_coords_otr, dim=0),
                all_atom_types_lyr=torch.stack(all_atom_types_lyr, dim=0),
                all_atom_types_otr=torch.stack(all_atom_types_otr, dim=0),
                all_pred_cart_coord_diff_lyr=torch.stack(
                    all_pred_cart_coord_diff_lyr, dim=0),
                all_pred_cart_coord_diff_otr=torch.stack(
                    all_pred_cart_coord_diff_otr, dim=0),
                all_noise_cart_lyr=torch.stack(all_noise_cart_lyr, dim=0),
                all_noise_cart_otr=torch.stack(all_noise_cart_otr, dim=0),
                is_traj=True))

        return output_dict

    def sample(self, num_samples, ld_kwargs):   #OK
        z = torch.randn(num_samples, self.hparams.hidden_dim,
                        device=self.device)
        samples = self.langevin_dynamics(z, ld_kwargs)
        return samples
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        (pred_num_atoms_lyr, pred_num_atoms_otr, pred_lengths_and_angles, pred_lengths, pred_angles, pred_N_lyr,
         pred_composition_per_atom_lyr, pred_composition_per_atom_otr) = self.decode_stats(
            z, batch.num_atoms_lyr, batch.num_atoms_otr, batch.lengths, batch.angles, teacher_forcing)  #TODO

        # sample noise levels.
        noise_level_lyr = torch.randint(0, self.sigmas.size(0),
                                    (batch.num_atoms_lyr.size(0),),
                                    device=self.device)
        noise_level_otr = torch.randint(0, self.sigmas.size(0),
                                    (batch.num_atoms_lyr.size(0),),
                                    device=self.device)
        used_sigmas_per_atom_lyr = self.sigmas[noise_level_lyr].repeat_interleave(batch.num_atoms, dim=0) # TODO batch.num_atoms = batch.num_atoms_lyr + batch.num_atoms_otr ?
        used_sigmas_per_atom_otr = self.sigmas[noise_level_otr].repeat_interleave(batch.num_atoms, dim=0)

        type_noise_level_lyr = torch.randint(0, self.type_sigmas.size(0),   # TODO: type_sigmas should be separated?
                                         (batch.num_atoms_lyr.size(0),),
                                         device=self.device) # TODO batch.num_atoms = batch.num_atoms_lyr + batch.num_atoms_otr ?
        type_noise_level_otr = torch.randint(0, self.type_sigmas.size(0),   # TODO: type_sigmas should be separated?
                                         (batch.num_atoms_otr.size(0),),
                                         device=self.device) # TODO batch.num_atoms = batch.num_atoms_lyr + batch.num_atoms_otr ?
        used_type_sigmas_per_atom_lyr = (
            self.type_sigmas[type_noise_level_lyr].repeat_interleave(
                batch.num_atoms_lyr, dim=0))    #TODO
        used_type_sigmas_per_atom_otr = (
            self.type_sigmas[type_noise_level_otr].repeat_interleave(
                batch.num_atoms_otr, dim=0))    #TODO

        # add noise to atom types and sample atom types.
        pred_composition_probs_lyr = F.softmax(
            pred_composition_per_atom_lyr.detach(), dim=-1)
        pred_composition_probs_otr = F.softmax(
            pred_composition_per_atom_otr.detach(), dim=-1)
        atom_type_probs_lyr = (
            F.one_hot(batch.atom_types_lyr - 1, num_classes=MAX_ATOMIC_NUM) +
            pred_composition_probs_lyr * used_type_sigmas_per_atom_lyr[:, None])
        atom_type_probs_otr = (
            F.one_hot(batch.atom_types_otr - 1, num_classes=MAX_ATOMIC_NUM) +
            pred_composition_probs_otr * used_type_sigmas_per_atom_otr[:, None])
        rand_atom_types_lyr = torch.multinomial(
            atom_type_probs_lyr, num_samples=1).squeeze(1) + 1
        rand_atom_types_otr = torch.multinomial(
            atom_type_probs_otr, num_samples=1).squeeze(1) + 1

        # add noise to the cart coords #? culd we do lyr and ootr together?
        cart_noises_per_atom_lyr = (
            torch.randn_like(batch.frac_coords) *
            used_sigmas_per_atom_lyr[:, None])
        cart_noises_per_atom_otr = (
            torch.randn_like(batch.frac_coords) *
            used_sigmas_per_atom_otr[:, None])
        cart_coords_lyr = frac_to_cart_coords(
            batch.frac_coords_lyr, pred_lengths, pred_angles, batch.num_atoms_lyr)
        cart_coords_otr = frac_to_cart_coords(
            batch.frac_coords_otr, pred_lengths, pred_angles, batch.num_atoms_otr)
        cart_coords_lyr = cart_coords_lyr + cart_noises_per_atom_lyr
        cart_coords_otr = cart_coords_otr + cart_noises_per_atom_otr
        noisy_frac_coords_lyr = cart_to_frac_coords(
            cart_coords_lyr, pred_lengths, pred_angles, batch.num_atoms_lyr)
        noisy_frac_coords_otr = cart_to_frac_coords(
            cart_coords_otr, pred_lengths, pred_angles, batch.num_atoms_otr)
        pred_cart_coord_diff_lyr, pred_cart_coord_diff_otr, pred_atom_types_lyr, pred_atom_types_otr = self.decoder(
            z, noisy_frac_coords_lyr, noisy_frac_coords_otr, rand_atom_types_lyr, rand_atom_types_otr, 
            batch.num_atoms_lyr, batch.num_atoms_otr, pred_lengths, pred_angles)
        # compute loss.
        num_atom_loss = self.num_atom_loss(pred_num_atoms_lyr+pred_num_atoms_otr, batch)
        lattice_loss = self.lattice_loss(pred_lengths_and_angles, batch)
        composition_loss = self.composition_loss(pred_composition_per_atom_lyr, batch.atom_types_lyr, batch) + \
                            self.composition_loss(pred_composition_per_atom_otr, batch.atom_types_otr, batch)
        
        coord_loss = self.coord_loss(pred_cart_coord_diff_lyr, noisy_frac_coords_lyr, used_sigmas_per_atom_lyr, batch) + \
                    self.coord_loss(pred_cart_coord_diff_otr, noisy_frac_coords_otr, used_sigmas_per_atom_otr, batch)
        type_loss = self.type_loss(pred_atom_types_lyr, batch.atom_types_lyr, used_type_sigmas_per_atom_lyr, batch) + \
                    self.type_loss(pred_atom_types_otr, batch.atom_types_otr, used_type_sigmas_per_atom_otr, batch)

        kld_loss = self.kld_loss(mu, log_var)   #TODO
        ...
        return {
            ...
            'kld_loss': kld_loss,
            'property_loss': property_loss,
            'pred_num_atoms': pred_num_atoms_lyr,
            'pred_num_atoms': pred_num_atoms_otr,
            'pred_lengths_and_angles': pred_lengths_and_angles,
            'pred_lengths': pred_lengths,
            'pred_angles': pred_angles,
            'pred_cart_coord_diff': pred_cart_coord_diff_lyr,
            'pred_cart_coord_diff': pred_cart_coord_diff_otr,
            'pred_atom_types': pred_atom_types_lyr,
            'pred_atom_types': pred_atom_types_otr,
            'pred_composition_per_atom': pred_composition_per_atom_lyr,
            'pred_composition_per_atom': pred_composition_per_atom_otr,
            'target_frac_coords': batch.frac_coords,
            'target_atom_types': batch.atom_types,
            'rand_frac_coords_lyr': noisy_frac_coords_lyr,
            'rand_frac_coords_otr': noisy_frac_coords_otr,
            'rand_atom_types': rand_atom_types_lyr,
            'rand_atom_types': rand_atom_types_otr,
            'z': z,
        }
