...
class BaseModule(pl.LightningModule):   #OK
...
class CrystGNN_Supervise(BaseModule):   #OK
    """
...
class CDVAE(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.fc_var = nn.Linear(self.hparams.latent_dim,#+self.hparams.esgo_dim,
                                self.hparams.latent_dim)    #OK   #? use embedded symmops
        self.fc_num_atoms = build_mlp(self.hparams.latent_dim+self.hparams.esgo_dim, self.hparams.hidden_dim,
                                      self.hparams.fc_num_layers, self.hparams.max_atoms+1) #OK   #? use embedded symmops
        self.fc_lattice = build_mlp(self.hparams.latent_dim+self.hparams.esgo_dim, self.hparams.hidden_dim,
                                    self.hparams.fc_num_layers, 6)  #OK   #? use embedded symmops
        self.fc_composition = build_mlp(self.hparams.latent_dim+self.hparams.esgo_dim, self.hparams.hidden_dim,
                                        self.hparams.fc_num_layers, MAX_ATOMIC_NUM) #OK   #? use embedded symmops
        # for property prediction.
        if self.hparams.predict_property:
            self.fc_property = build_mlp(self.hparams.latent_dim+self.hparams.esgo_dim, self.hparams.hidden_dim,
                                         self.hparams.fc_num_layers, 1)     #OK   #? use embedded symmops

        sigmas = torch.tensor(np.exp(np.linspace(
            np.log(self.hparams.sigma_begin),
            np.log(self.hparams.sigma_end),
            self.hparams.num_noise_level)), dtype=torch.float32)    #OK

        self.sigmas = nn.Parameter(sigmas, requires_grad=False) #OK
        ...
        self.type_sigmas = nn.Parameter(type_sigmas, requires_grad=False)   #OK

        self.embedding = torch.zeros(100, 92)
        for i in range(100):
            self.embedding[i] = torch.tensor(KHOT_EMBEDDINGS[i + 1])    #OK

        # obtain from datamodule.
        self.lattice_scaler = None
        self.scaler = None

    def reparameterize(self, mu, logvar):   #OK
        """
    ...
    def decode_stats(self, z, esgo, gt_num_atoms=None, gt_lengths=None, gt_angles=None,
        ...
        if gt_num_atoms is not None:
            num_atoms = self.predict_num_atoms(z_esgo)    #? how to use space group as the input.
            lengths_and_angles, lengths, angles = (
                self.predict_lattice(z_esgo, gt_num_atoms))    #? how to use space group as the input.
            composition_per_atom = self.predict_composition(z_esgo, gt_num_atoms)    #? how to use space group as the input.
            if self.hparams.teacher_forcing_lattice and teacher_forcing:
                lengths = gt_lengths
                angles = gt_angles
        else:
            num_atoms = self.predict_num_atoms(z_esgo).argmax(dim=-1)    #? how to use space group as the input.
            lengths_and_angles, lengths, angles = (
                self.predict_lattice(z_esgo, num_atoms)) #? how to use space group as the input.
            composition_per_atom = self.predict_composition(z_esgo, num_atoms)   #? how to use space group as the input.
        return num_atoms, lengths_and_angles, lengths, angles, composition_per_atom

    @torch.no_grad()
    def langevin_dynamics(self, z, esgo, ld_kwargs, gt_num_atoms=None, gt_atom_types=None):   #? use space group
        """
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        # add noise to atom types and sample atom types.    #OK
        ...
        # add noise to the cart coords  #OK
        ...
        pred_cart_coord_diff, pred_atom_types = self.decoder(
            z, esgo, noisy_frac_coords, rand_atom_types, batch.num_atoms, pred_lengths, pred_angles)  #? use esgo

        # compute loss.
        num_atom_loss = self.num_atom_loss(pred_num_atoms, batch)   #OK
        lattice_loss = self.lattice_loss(pred_lengths_and_angles, batch)    #OK
        composition_loss = self.composition_loss(
            pred_composition_per_atom, batch.atom_types, batch) #OK
        coord_loss = self.coord_loss(
            pred_cart_coord_diff, noisy_frac_coords, used_sigmas_per_atom, batch)   #OK
        type_loss = self.type_loss(pred_atom_types, batch.atom_types,
                                   used_type_sigmas_per_atom, batch)    #OK

        kld_loss = self.kld_loss(mu, log_var)   #OK
        
        if self.hparams.use_sgo_loss:   #!
            sgo_loss = sgo_loss()   #? how to define the space group loss? 
        else:
            sgo_loss = 0.
    ...
    def generate_rand_init(self, pred_composition_per_atom, pred_lengths,
                           pred_angles, num_atoms, batch):  #OK
    ...
    def sample_composition(self, composition_prob, num_atoms):  #OK
        """
    ...
    def predict_num_atoms(self, z): #OK #? use space group operations
        return self.fc_num_atoms(z)

    def predict_property(self, z): #OK #? use space group operations
        self.scaler.match_device(z)
        return self.scaler.inverse_transform(self.fc_property(z))

    def predict_lattice(self, z, num_atoms): #OK #? use space group operations
    ...
    def predict_composition(self, z, num_atoms): #OK #? use space group operations
        z_per_atom = z.repeat_interleave(num_atoms, dim=0)
        pred_composition_per_atom = self.fc_composition(z_per_atom)
        return pred_composition_per_atom

    def num_atom_loss(self, pred_num_atoms, batch): #OK
        return F.cross_entropy(pred_num_atoms, batch.num_atoms)

    def property_loss(self, z, batch):  #OK
        return F.mse_loss(self.fc_property(z), batch.y)

    def lattice_loss(self, pred_lengths_and_angles, batch): #OK
    ...
    def composition_loss(self, pred_composition_per_atom, target_atom_types, batch):    #OK
    ...
    def kld_loss(self, mu, log_var):    #OK
        kld_loss = torch.mean(
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> torch.Tensor:  #OK
    ...
    def test_step(self, batch: Any, batch_idx: int) -> torch.Tensor:    #OK
        outputs = self(batch, teacher_forcing=False, training=False)
    ...
    def compute_stats(self, batch, outputs, prefix):    #OK
