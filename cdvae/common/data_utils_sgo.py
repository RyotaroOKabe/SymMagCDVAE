...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    def process_one(row, niggli, primitive, graph_method, prop_list):
        ...
        #!
        # row['sgo'] = row['sgo'].map(lambda x: np.fromstring(x.replace('[', ' ').replace(']', ' ').replace('\n', ' '), sep=' ').reshape(-1, 3, 3))
        oprs = row['sgo']#[0]
        sgn = row['spacegroup.number']
        sgo_dict = {'oprs': oprs, 'sgn': sgn}
        result_dict.update(sgo_dict)
        #!
        return result_dict

    df['sgo'] = df['sgo'].map(lambda x: np.fromstring(x.replace('[', ' ').replace(']', ' ').replace('\n', ' '), sep=' ').reshape(-1, 3, 3))
