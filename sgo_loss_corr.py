...
from cdvae.pl_modules.space_group_cart import *
...
lattice = torch.tensor(pstruct.lattice.matrix)
...
loss_prod1 = sgo_loss_cart(frac, sgo, r_max, lattice)
loss_perm1 = sgo_loss_perm_cart(frac, sgo, r_max, lattice)
loss_prod2 = sgo_loss_cart2(frac, sgo, r_max, lattice)
loss_perm2 = sgo_loss_perm_cart2(frac, sgo, r_max, lattice)
...
loss_prod = sgo_cum_loss_cart(frac0, oprs, r_max, lattice)
loss_prod.backward()
...
loss_perm = sgo_cum_loss_perm_cart(frac1, oprs, r_max, lattice)
loss_perm.backward()
...
frac0 = torch.tensor(pstruct.frac_coords)
frac0.requires_grad = grad
loss_prod2 = sgo_cum_loss_cart2(frac0, oprs, r_max, lattice)
loss_prod2.backward()
print('loss_prod: ', loss_prod2)
print('loss_prod.grad: ', loss_prod2.grad)
print('frac0.grad: ', frac0.grad)

frac1 = torch.tensor(pstruct.frac_coords)
frac1.requires_grad = grad
loss_perm2 = sgo_cum_loss_perm_cart2(frac1, oprs, r_max, lattice)
loss_perm2.backward()
print('loss_perm: ', loss_perm2)
print('loss_perm.grad: ', loss_perm2.grad)
print('frac1.grad: ', frac1.grad)
...
for i, l in enumerate(logvars):
    ...
    loss_0, loss_1, loss_2, loss_3, loss_4 , loss_5, loss_6 = torch.zeros(1),torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1)
    grads_0, grads_1, grads_2, grads_3, grads_4, grads_5, grads_6 = torch.zeros(1),torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1), torch.zeros(1)
    ...
    for j, dstruct in enumerate(dstructs):
        ...
        frac.requires_grad = grad
        cart = torch.tensor(data=dstruct.cart_coords)
        cart.requires_grad = grad
        lattice = torch.tensor(dstruct.lattice.matrix)
        frac0, frac1, frac2, frac3, frac4, frac5, frac6 = torch.tensor(dstruct.frac_coords), torch.tensor(dstruct.frac_coords), torch.tensor(dstruct.frac_coords), \
                                                torch.tensor(dstruct.frac_coords), torch.tensor(dstruct.frac_coords), torch.tensor(dstruct.frac_coords), torch.tensor(dstruct.frac_coords)
        frac0.requires_grad, frac1.requires_grad = grad, grad
        loss0 = sgo_cum_loss_cart(frac0, oprs, r_max, lattice)
        # ys0.append(loss0)
        loss0.backward()
        grads0 = frac0.grad
        frac0_list.append(frac0.detach().numpy())
        grads0_list.append(-grads0.detach().numpy())
        loss1 = sgo_cum_loss_perm_cart(frac1, oprs, r_max, lattice)
        # ys1.append(loss1)
        loss1.backward()
        grads1 = frac1.grad
        frac1_list.append(frac1.detach().numpy())
        grads1_list.append(-grads1.detach().numpy())
        loss2 = sgo_cum_loss_cart2(frac2, oprs, r_max, lattice)
        # ys2.append(loss2)
        loss2.backward()
        # grads2 = frac2.grad
        frac2_list.append(frac2.detach().numpy())
        # grads2_list.append(-grads2.detach().numpy())
        loss3 = sgo_cum_loss_perm_cart2(frac3, oprs, r_max, lattice)
        # ys3.append(loss3)
        loss3.backward()
        # grads3 = frac3.grad
        frac3_list.append(frac3.detach().numpy())
        # grads3_list.append(-grads3.detach().numpy())
        loss4 = sgo_cum_loss_gw_cart(frac4, oprs, lattice)
        # ys3.append(loss3)
        loss4.backward()
        # grads4 = frac4.grad
        frac4_list.append(frac4.detach().numpy())
        # grads4_list.append(-grads4.detach().numpy())
...
ax.plot(logvars, ys0/max(ys0), label='prod1')
ax.plot(logvars, ys1/max(ys1), label='perm1')
ax.plot(logvars, ys2/max(ys2), label='prod2')
ax.plot(logvars, ys3/max(ys3), label='perm2')
